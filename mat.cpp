...
int main(int,char**)
{
    ...
    printf("%u %u %u %u %u\n", CV_MAT_DEPTH_MASK, 1 << 3, (1<<3) - 1, CV_8UC3, CV_8UC(2));

    Mat AA, CC;                          // creates just the header parts
    AA = imread("lena.jpg", IMREAD_COLOR); // here we'll know the method used (allocate matrix)

    Mat BB(AA);                          // Use the copy constructor
    CC = AA;
